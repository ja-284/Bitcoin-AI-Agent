...
| research feature definitions | **feature fingerprint** (sha256 of feature values on a fixed reference series) | computed by `agent/shadow/features.py::feature_fingerprint` | inside each model artefact (`feature_fingerprint`) | any change to `agent/research/features.py` / `microstructure.py` that alters a value | `load_model()` recomputes it and **refuses to run old coefficients on changed features** (`ModelVersionError`); `tests/test_shadow.py` |
...
| database schema | `SCHEMA_VERSION` = 3 | `agent/database/db.py`; written to `schema_meta` by `init_schema()` | one row per database | any migration | `tests/test_scoring_golden.py` pins code = SQL; the weekly report prints database vs code and flags a mismatch |
