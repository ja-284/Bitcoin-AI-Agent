...
| G | Safe historical research | **PARTIALLY COMPLETE** | E025 (news power), E026 (calibration by volatility regime), **E027 done 2026-09-26** (calibration by weekday/weekend and hour block: offsetting intraday miscalibration — night over-stated, US session under-stated; the 2,000 h slices will be read against these offsets) | **G2** research-only: does the intraday miscalibration come from the single calendar harmonic (the E027 mechanism)? — hypothesis-generating on development data, for a possible future model version only |
