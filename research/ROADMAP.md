...
- [ ] **PHASE 12 — Final untouched holdout evaluation** (once) — **PREPARED 2026-09-21, NOT RUN.** Pre-registered in `research/experiments/E014_holdout_evaluation.json` (objects A–D, pass rules copied unchanged from E001/E011/E012/E013, what counts as confirmation). Script `agent/research/holdout_eval.py`; its `--dry-run` reproduced the validation-period results end-to-end (`research/results/E014_dryrun/`) without opening the holdout (`research/HOLDOUT_ACCESS.log` still does not exist). **Unsealing is irreversible and waits for the user's explicit go-ahead** (`python -m agent.research.holdout_eval --unseal`).
