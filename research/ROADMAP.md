...
> **CURRENT STATE: read this first (2026-09-26 evening).** The project is in **PRE-500H MONITORING-ONLY
> MODE**: the pre-500h plan is complete (`research/INTERIM_PLAN_AUDIT.md`), the predictive path is frozen
> for the current prospective evaluation, and no new experiment starts unless new live evidence justifies
> one. Operational status, alarms and the checkpoint procedure: `docs/ops/STATUS.md`. The checkpoint rules:
> `research/LIVE_EVALUATION.md`. Everything below is a **dated history**. Any "resume point", "first thing
> tomorrow", "next actions" or "open user action" in an older section is superseded. The heartbeat and the
> least-privilege role are done (2026-09-25), the schema is version 4, and the live scoring is 0.2.0.
...
- [x] **PHASE 8 — New feature groups** (complete 2026-09-20 except 8.6, deferred until the live news archive is large enough; 8.8 unavailable). Outcome: 47 candidate features across 6 groups; **no directional feature meets the criterion with a usable size**; one consistent tiny *reversal* family at 1–6h (momentum, volume, taker-buy share; |ρ| ≤ 0.05); **two strong move-size predictors** (recent volatility, trade intensity) plus a time-of-day effect. Watch list for live re-tests: funding (24h contrarian), dollar/yield 5-day (168h negative).
  ...
  - [ ] 8.6 News improvements — **deferred**: only the live archive is usable (no history; LLM-knowledge leakage), and it is far too small (tens of hours). Re-opens when ≥ 500 live hours with news exist (~3 weeks after go-live). *(E025, 2026-09-24: 500 usable hours = a descriptive first look only; a verdict needs ~3,140 usable hours. See J below.)*
...
- **Evening: final documentation and consistency pass; the project enters PRE-500H MONITORING-ONLY MODE.**
  - STATUS.md got one dated snapshot whose figures name their source commands. The prospective count
    comes from the checkpoint command itself.
  - `tests/test_status_docs.py` keeps STATUS.md, status.json and the code's versions in agreement.
  - LIVE_EVALUATION.md gathers the 500-hour procedure in one place and records E026–E029 as reading
    context, with no rule changed.
  - Stale "open" / "not set up" / version statements were corrected across the README, security,
    observability, open user actions, the readiness gate and CLAUDE.md.
  - The user's long-term execution goal is recorded as **NOT ACTIVE** (`docs/FUTURE_EXECUTION_ARCHITECTURE.md`).
  - One claim corrected: the free EWMA reference's candle warm-up reaches into the holdout period's last
    weeks. No holdout performance is computed, and the weight is below 2e-10. It is documented precisely
    and not changed.
  - Mutation run 45/45; 480 unit tests; 110 of 500 prospective hours at 15:45 UTC.
  - Nothing predictive, no checkpoint rule, no holdout evaluation.
