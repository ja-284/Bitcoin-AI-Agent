...
- **Scheduling (2026-09-19/20):** GitHub's own cron ran 2 of 8 slots on day one; with four slots/hour it covered ~9 of 11 hours overnight. The reliable path is an external trigger: Supabase `pg_cron` + `pg_net` posts to GitHub's `workflow_dispatch` API at :12 each hour, token in Supabase Vault (`docs/ops/external_trigger.md`). The GitHub fine-grained token `supabase-dispatch` **expires 2027-09-20 — regenerate and re-store it before then.** Healthchecks.io heartbeat still not set up.
